...
if mcflag==0:
    for j in arange(Nj):
        ...
        for i in arange(Ni):
            ...
            T1[i,j] = sim[0,0]
            STD1[i,j] = sim[0,1]
    np.save('fig1_data_T1b', T1)
    np.save('fig1_data_STD1b',STD1)
else:
    T1 = np.load('fig1_data_T1b.npy')
    STD1 = np.load('fig1_data_STD1b.npy')
...
if mcflag==0:
    for i in arange(Ni):
        ...
        for j in arange(Nj):
            ...
            T2[i,j] = sim[0,0]
            STD2[i,j] = sim[0,1]
    np.save('fig1_data_T2b', T2)
    np.save('fig1_data_STD2b',STD2)   
else:
    T2 = np.load('fig1_data_T2b.npy')
    STD2 = np.load('fig1_data_STD2b.npy')
...
def apprx(alpha,beta):
    ...
    #Dk = D
    ### higher order QSS
    lam = lam0*(1.-a/beta*lam0)
    Dk = D + k*a*b*(b+1)/(2.*beta**2)
    ### decoupling for T_0
    #lam = lam0
    #Dk = D/kappa
    ...
    Tx = 1./lam + a1*a2/(2*pi)*tD*  special.iv(0,lDs)/special.iv(1,lDs)/lDs;  # Time to target, starting at the target
...
plot(alphag1[:,0],Ta1[:,0],'k',lw=2,label=r'$\beta=1$')
